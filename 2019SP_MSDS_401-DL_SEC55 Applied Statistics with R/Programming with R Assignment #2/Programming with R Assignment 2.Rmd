...
mean <- round(sum(outcome * prob), digits = 2)
mean

variance <- round(((sum((outcome^2) * prob)) - (mean^2)), digits = 2)
variance
```
...
points(faithful_1$waiting, faithful_1$eruptions,col = "dark red", pch = 16)
...
***Answer: As waiting time increases the erutption time also increases. There is positive correlation between eruption time and waiting time.***
...
plot(x = pre, y = post, 
     main = "Scatterplot Pre vs.Post", xlab = "Variable Pre", ylab = "Variable Post")
...
U_Bound <- d_bar - t_table * sd / sqrt(n)
U_Bound

L_bound <- d_bar + t_table * sd / sqrt(n)
L_bound

##check:
t.test(post, pre, alternative = "less", paired = TRUE, conf.level = 0.95)
```
